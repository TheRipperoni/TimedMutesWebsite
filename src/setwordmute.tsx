    ...
    fetch(import.meta.env.VITE_API_HOST + '/timed-mute-word', requestOptions)
      .then(response => {
        if (response.status == 200) {
          ...
          setSnackbarMessage(`Word muted for ${durationLabel}`)
        ...
        console.error('Failed to submit word mute:', err);
        alert('Network error submitting word mute');
        setLoading(false)
      });
  }
  ...
  return (
    <Box sx={{
      ...
      <FormControl fullWidth variant="outlined" sx={{gap: 3}}>
        ...
        >
          {loading ? <CircularProgress size={24} color="inherit"/> : "Mute Word"}
        </Button>
      </FormControl>
      <Snackbar
        open={snackbarOpen}
